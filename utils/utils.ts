...
export const getRandomPokemonName = (pokemonNames: string[], maxLength: number): string => {

  const pokemonName = getRandomItemFromArray(pokemonNames)

  if (pokemonName.length === maxLength) return pokemonName

  const filteredPokemonNames = pokemonNames.filter(pokemon => pokemon !== pokemonName)
